import init, { greet, update } from "./pkg/GoLtest.js"
    ...
    while (true) {
      ...
      cells = cells.map((col, column) =>
        col.map((cell, row) => {
          ...
          if (tcell > 1) tcell = 1;
          y[1] += tcell;
          let rounded_tcell = Math.round(tcell * 100) / 100;
      ...
      */
      cells = [].slice().call(update(wasm, new Float64Array([].concat(...cells)), columns))
